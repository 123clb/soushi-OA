...
/**
 * 这里是和treeview整合之后，在关闭navTab之后，刷新左边的树，至于右边的列表则在jquery.treeview.async.js内66行进行
 * @param json
 * @returns
 * @author <p>Innate Solitary 于 2012-2-2 上午9:00:14</p>
 */
function treeReloadNavTabAjaxDone(json) {
  var result = navTabAjaxDone(json);
  if(json.treeId) {
    $("#" + json.treeId).treeview({reload:json.treeNodeId});
  }
  return result;
}

function treeReloadDialogAjaxDone(json) {
  var result = dialogAjaxDone(json);
  if(json.treeId) {
    $("#" + json.treeId).treeview({reload:json.treeNodeId});
  }
  return result;
  ...
  ajaxTodo:function($p){
    return this.each(function(){
      ...
      $this.click(function(event){
        ...
        if (title) {
          // add by wangyb 增加提示信息框多个按钮的问题 2011-12-22
          //调用例子
          //<li><a class="edit" fresh=true operator="[{name:'审核通过',url:'course/course/delete.do?courseId={sid_course}'},{name:'审核不通过',url:'course/course/delete.do?courseId={sid_course}'}]" target="ajaxTodo" title="确定要审核吗?" ><span>审核</span></a></li>
          var operator = $this.attr("operator");
          if(operator){
            opers = DWZ.jsonEval(operator);
            var buttons = new Array();
            for(var i = 0; i < opers.length; i++) {
              var oper = opers[i];
              var todoUrl = unescape(oper.url).replaceTmById($(event.target).parents(".unitBox:first"));
              var button =  {name: oper.name,call:function(){ajaxTodo(todoUrl, $this.attr("callback"));} }; 
              buttons.push(button);
            }
            var sysButtons = {okName:$.regional.alertMsg.butMsg.ok, okCall:null, cancelName:$.regional.alertMsg.butMsg.cancel, cancelCall:null};
            buttons.push({name: sysButtons.cancelName,call:sysButtons.cancelCall}); 
            alertMsg.customconfirm(title, buttons);
          }else{
